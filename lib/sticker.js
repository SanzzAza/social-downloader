 ...
 * @param {string} [opts.imageUrl]  optional picture on the right side
...
async function renderWindowSticker(opts = {}) {
  ...
  if (imageUrl) {
    try {
      ...
      ctx.drawImage(img, cX + cW - dw - cW * 0.03, cY + (cH - dh) / 2, dw, dh);
      textBoxW = cW - boxW - cW * 0.05;
    } catch (_) {
      // bad/unreachable image -> just render text full width
    }
  }
  ...
  const { lines, size: fs, lineHeight } = fitText(
    ctx, text, maxW, maxH, Math.round(cH * 0.3)
  );
  ...
  let ty = cY + (cH - subReserve - blockH) / 2;
